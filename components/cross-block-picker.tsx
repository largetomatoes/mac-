...
export function CrossBlockPicker({data,editing,saving,error,onClose,onSave}:{data:Notebook;editing?:CrossThought;saving:boolean;error:string;onClose:()=>void;onSave:(title:string,ids:string[])=>Promise<void>}){
 ...
 const blocks=data.notes.filter(n=>n.kind==='question').sort((a,b)=>number(a.id).localeCompare(number(b.id),undefined,{numeric:true}));
 const label=(id:string)=>{const n=data.notes.find(n=>n.id===id);return n?`${number(id)||'核心问题'} ${n.title}`:'原有思考连接';};
 ...
 <div className="block-options">{blocks.filter(n=>label(n.id).includes(query.trim())).map(n=><label key={n.id} className={'block-option '+(ids.includes(n.id)?'is-selected':'')} style={{paddingLeft:14+Math.min(4,number(n.id).split('.').length-1)*18}}><Checkbox checked={ids.includes(n.id)} onCheckedChange={()=>toggle(n.id)} disabled={saving||(!ids.includes(n.id)&&ids.length>=30)} aria-label={label(n.id)}/><span className="block-number">{number(n.id)||'核心'}</span><span>{n.title}</span></label>)}{!blocks.some(n=>label(n.id).includes(query.trim()))&&<p>没有匹配的板块</p>}</div>
