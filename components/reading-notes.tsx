...
import type { Notebook, ReadingNote, Thought } from "@/lib/notebook";
...
export function ReadingNotes({ data, questionId, saving, save }: Props) {
  ...
  return <section className="reading-section">
    <div className="reading-heading"><h3><BookOpen size={17}/>阅读笔记 <span>{notes.length}</span></h3><Button variant="outline" size="sm" onClick={() => setCreating(true)}><Plus/>摘录原文</Button></div>
    {!groups.length && <p className="reading-empty">暂无阅读笔记</p>}
    <div className="book-groups">{groups.map((group) => <section className="book-group" key={`${group.book}|${group.author}`}>
      ...
      {Array.from(group.chapters.values()).map((chapterGroup) => <details className="chapter-group" key={chapterGroup.chapter} open>
    ...
    <Sheet open={!!selected} onOpenChange={(open) => { if (!open) setSelectedId(null); }}><SheetContent className="reading-sheet"><div className="reading-sheet-head"><span>{selected?.book} · {selected?.chapter}</span><SheetTitle>阅读笔记</SheetTitle><SheetDescription>{selected?.author}{selected?.locator ? ` · ${selected.locator}` : ""}</SheetDescription></div>{selected && <div className="reading-sheet-scroll">
      ...
      {!!selected.thoughts.length && <section className="reading-thoughts"><h3>后来想到的</h3>{selected.thoughts.map((entry) => <article key={entry.id}><small>{when(entry.at)} · {entry.origin}</small><p>{entry.text}</p>{entry.reason && <aside><strong>为什么改变判断</strong><p>{entry.reason}</p></aside>}</article>)}</section>}
