...
export function ReadingNotes({ data, questionId, saving, save, onOpenBook, initialId, initialThoughtId, onClose }: Props) {
  ...
  const groups = useMemo(() => {
    ...
    for (const note of notes) {
      ...
      const chapterGroup = bookGroup.chapters.get(chapterKey) || { chapter: clean(note.chapter) || "未标章节", notes: [] as ReadingNote[] };
  ...
  return <>
    ...
    <Dialog open={creating} onOpenChange={setCreating}><DialogContent className="reading-create-dialog"><DialogTitle>摘录原文</DialogTitle><DialogDescription>每段原文独立保存，同一本书的同一章节会自动集合。</DialogDescription><form onSubmit={createNote} className="reading-create-form">
      ...
      <div className="source-fields"><label>章节（可选）<Input list="reading-chapters" value={chapter} onChange={(e) => setChapter(e.target.value)}/><datalist id="reading-chapters">{chapters.map((value) => <option key={value} value={value}/>)}</datalist></label><label>页码或段落编号<Input value={locator} onChange={(e) => setLocator(e.target.value)}/></label></div>
