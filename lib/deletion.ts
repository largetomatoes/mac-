...
export function deletionPlan(data:Notebook,id:string){
 ...
 const keptReadingNotes=data.readingNotes.map(n=>({...n,questionIds:n.questionIds.filter(id=>!ids.has(id))}));
 ...
 return {target,ids,notes,links,readingNotes,crossThoughts,next:{...data,notes:data.notes.filter(n=>!ids.has(n.id)),readingNotes:keptReadingNotes,bookThoughts:data.bookThoughts.map(entry=>({...entry,questionIds:entry.questionIds.filter(id=>!ids.has(id))})),crossThoughts:keptCrossThoughts,thoughtReplies:data.thoughtReplies.filter(r=>!thoughtIds.has(r.thoughtId)),manuscripts:data.manuscripts.map(m=>({...m,links:m.links.filter(link=>link.targetKind==='reading'||!ids.has(link.questionId))})),links:data.links.filter(l=>!ids.has(l.from)&&!ids.has(l.to)),dismissedSuggestions:data.dismissedSuggestions.filter(s=>!Array.from(ids).some(id=>s.endsWith(':'+id)))}};
}
