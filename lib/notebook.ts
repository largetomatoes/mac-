...
export type OcrCacheEntry = { libraryBookId:string; page:number; text:string; at:string };
...
export type Notebook = { notes:Note[]; links:Link[]; cards:Card[]; readingNotes:ReadingNote[]; crossThoughts:CrossThought[]; thoughtReplies:ThoughtReply[]; manuscripts:Manuscript[]; libraryBooks:LibraryBook[]; libraryHighlights:LibraryHighlight[]; ocrCache:OcrCacheEntry[]; dismissedSuggestions:string[] };
...
],links:[],cards:[exampleCard],readingNotes:[],crossThoughts:[],thoughtReplies:[],manuscripts:[],libraryBooks:[],libraryHighlights:[],ocrCache:[],dismissedSuggestions:[] };
export function normalize(data:Partial<Notebook>):Notebook {
 ...
 return {...initialNotebook,...data,notes:(data.notes||initialNotebook.notes).map(n=>({...n,thoughts:n.thoughts||[]})),cards:(data.cards??[exampleCard]).map(c=>({...c,thoughts:c.thoughts||[]})),readingNotes:(data.readingNotes||[]).map(n=>({...n,thoughts:n.thoughts||[]})),crossThoughts,thoughtReplies:data.thoughtReplies||[],manuscripts:(data.manuscripts||[]).map(m=>({...m,links:(m.links||[]).map(link=>({...link,targetKind:link.targetKind||'question'})),revisions:(m.revisions||[]).map(r=>({...r,links:(r.links||[]).map(link=>({...link,targetKind:link.targetKind||'question'}))}))})),libraryBooks:(data.libraryBooks||[]).map(book=>({...book,author:book.author||'',progress:book.progress||{}})),libraryHighlights:data.libraryHighlights||[],ocrCache:data.ocrCache||[],dismissedSuggestions:data.dismissedSuggestions||[]};
}
