...
export type Notebook = { notes:Note[]; links:Link[]; cards:Card[]; dismissedSuggestions:string[] };
...
],links:[],cards:[exampleCard],dismissedSuggestions:[] };
export function normalize(data:Partial<Notebook>):Notebook {return {...initialNotebook,...data,notes:(data.notes||initialNotebook.notes).map(n=>({...n,thoughts:n.thoughts||[]})),cards:data.cards??[exampleCard],dismissedSuggestions:data.dismissedSuggestions||[]};}
