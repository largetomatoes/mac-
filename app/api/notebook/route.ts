...
const thoughtReply=z.object({id:z.string().min(1).max(100),thoughtId:z.string().min(1).max(100),text:z.string().trim().min(1).max(100000),at:z.string().max(100)});
...
const readingNote=z.object({id:z.string().min(1).max(100),questionIds:z.array(z.string().min(1).max(100)).max(100),book:z.string().trim().min(1).max(500),author:z.string().trim().max(300),chapter:z.string().trim().max(500),locator:z.string().max(300),quote:z.string().trim().min(1).max(100000),interpretation:z.string().trim().min(1).max(100000),at:z.string().max(100),thoughts:z.array(thought).max(1000),libraryBookId:z.string().max(100).optional(),sourceLocation:z.string().max(2000).optional(),anchorY:z.number().min(0).max(1).optional()});
...
const manuscriptLink=z.object({id:z.string().min(1).max(100),questionId:z.string().min(1).max(100),targetKind:z.enum(['question','reading']).optional(),quote:z.string().max(100000),start:z.number().int(),end:z.number().int(),prefix:z.string().max(1000),suffix:z.string().max(1000),at:z.string().max(100),unresolved:z.boolean().optional()});
const manuscriptRevision=z.object({title:z.string().max(300),body:z.string().max(500000),links:z.array(manuscriptLink).max(5000),at:z.string().max(100)});
const manuscript=z.object({id:z.string().min(1).max(100),title:z.string().trim().min(1).max(300),body:z.string().max(500000),links:z.array(manuscriptLink).max(5000),at:z.string().max(100),updatedAt:z.string().max(100),revisions:z.array(manuscriptRevision).max(2000)});
const pdfChapter=z.object({id:z.string().min(1).max(100),title:z.string().trim().min(1).max(500),startPage:z.number().int().positive()});
const libraryBook=z.object({id:z.string().min(1).max(100),title:z.string().trim().min(1).max(500),author:z.string().max(300),format:z.enum(['pdf','epub']),storedFile:z.string().min(1).max(500),originalName:z.string().min(1).max(500),addedAt:z.string().max(100),progress:z.object({page:z.number().int().positive().optional(),totalPages:z.number().int().positive().optional(),cfi:z.string().max(4000).optional(),percentage:z.number().min(0).max(1).optional(),rotation:z.union([z.literal(0),z.literal(90),z.literal(180),z.literal(270)]).optional()}),pdfChapters:z.array(pdfChapter).max(2000).optional()});
const libraryHighlight=z.object({id:z.string().min(1).max(100),libraryBookId:z.string().min(1).max(100),quote:z.string().trim().min(1).max(100000),locator:z.string().max(1000),sourceLocation:z.string().max(4000),at:z.string().max(100)});
const ocrLine=z.object({text:z.string().max(10000),x0:z.number().min(0).max(1),y0:z.number().min(0).max(1),x1:z.number().min(0).max(1),y1:z.number().min(0).max(1)});
const ocrEntry=z.object({libraryBookId:z.string().min(1).max(100),page:z.number().int().positive(),text:z.string().max(200000),lines:z.array(ocrLine).max(10000).optional(),at:z.string().max(100)});
const bookThought=z.object({id:z.string().min(1).max(100),libraryBookId:z.string().min(1).max(100).optional(),bookTitle:z.string().trim().min(1).max(500),text:z.string().trim().min(1).max(100000),scope:z.enum(['book','location']),locator:z.string().max(1000),sourceLocation:z.string().max(4000).optional(),quote:z.string().max(100000).optional(),chapter:z.string().max(500).optional(),questionIds:z.array(z.string().min(1).max(100)).max(100),at:z.string().max(100),thoughts:z.array(thought).max(1000)});
const schema=z.object({version:z.number().int().nonnegative(),data:z.object({notes:z.array(note).min(1).max(3000),cards:z.array(card).max(5000),readingNotes:z.array(readingNote).max(10000),bookThoughts:z.array(bookThought).max(10000),crossThoughts:z.array(crossThought).max(10000),thoughtReplies:z.array(thoughtReply).max(30000),manuscripts:z.array(manuscript).max(5000),libraryBooks:z.array(libraryBook).max(5000),libraryHighlights:z.array(libraryHighlight).max(50000),ocrCache:z.array(ocrEntry).max(50000),links:z.array(z.object({id:z.string(),from:z.string(),to:z.string(),type:z.enum(['相关','支持','反驳','启发'])})).max(10000),dismissedSuggestions:z.array(z.string()).max(15000)})});
...
export async function PUT(req:Request){try{
 ...
 const old=normalize(JSON.parse(current.content));
	 const records=[...data.notes,...data.cards,...data.crossThoughts,...data.readingNotes,...data.bookThoughts,...data.thoughtReplies];const allIds=new Set(records.map(n=>n.id));const ids=new Set([...data.notes,...data.cards,...data.crossThoughts].map(n=>n.id));if(allIds.size!==records.length)return response('记录标识重复。',400);
 ...
 for(const r of data.readingNotes)if(r.questionIds.some(id=>!data.notes.some(n=>n.id===id&&n.kind==='question')&&!data.crossThoughts.some(c=>c.id===id)))return response('阅读笔记需要属于有效的问题。',400);
	 const bookIds=new Set(data.libraryBooks.map(book=>book.id));if(bookIds.size!==data.libraryBooks.length)return response('书籍标识重复。',400);
	 if(data.libraryHighlights.some(highlight=>!bookIds.has(highlight.libraryBookId))||data.ocrCache.some(entry=>!bookIds.has(entry.libraryBookId)))return response('高亮或识别文字没有对应的书籍。',400);
	 const readingIds=new Set(data.readingNotes.map(note=>note.id)),questionIds=new Set([...data.notes.filter(note=>note.kind==='question').map(note=>note.id),...data.crossThoughts.map(cross=>cross.id)]);
	 for(const entry of data.bookThoughts)if((entry.libraryBookId&&!bookIds.has(entry.libraryBookId))||entry.questionIds.some(id=>!questionIds.has(id)))return response('本书思考的书籍或关联问题无效。',400);
 for(const manuscript of data.manuscripts)for(const link of manuscript.links){const validTarget=link.targetKind==='reading'?readingIds.has(link.questionId):questionIds.has(link.questionId);if(!validTarget)return response('文稿中有一处关联已经失效。',400);if(!link.unresolved&&(link.start<0||link.end>manuscript.body.length||link.start>=link.end))return response('文稿中有一处关联位置无效。',400);}
	 const thoughtIds=new Set([...data.notes.flatMap(n=>(n.thoughts||[]).map(t=>t.id)),...data.cards.flatMap(c=>c.thoughts.map(t=>t.id)),...data.crossThoughts.flatMap(c=>c.thoughts.map(t=>t.id)),...data.readingNotes.flatMap(r=>r.thoughts.map(t=>t.id)),...data.bookThoughts.flatMap(r=>r.thoughts.map(t=>t.id))]);
 if(new Set(data.thoughtReplies.map(r=>r.thoughtId)).size!==data.thoughtReplies.length||data.thoughtReplies.some(r=>!thoughtIds.has(r.thoughtId)))return response('每条思考只能保留一份有效回答。',400);
 ...
 for(const n of old.notes){const next=data.notes.find(x=>x.id===n.id);if(!next||!same({...n,thoughts:[]},{...next,thoughts:[]})||!same(n.thoughts,next.thoughts.slice(0,n.thoughts?.length||0)))return response('原有问题与回答需要保留，请追加新的思考。',400);}
	 ...
	 for(const entry of old.bookThoughts){const next=data.bookThoughts.find(n=>n.id===entry.id);if(!next||!same({...entry,thoughts:[]},{...next,thoughts:[]})||!same(entry.thoughts,next.thoughts.slice(0,entry.thoughts.length)))return response('本书思考和已有记录需要保留，请追加新的思考。',400);}
 ...
 for(const reply of old.thoughtReplies){const next=data.thoughtReplies.find(r=>r.id===reply.id);if(!next||!same(reply,next))return response('已有回答需要保留。',400);}
