...
const readingNote=z.object({id:z.string().min(1).max(100),questionIds:z.array(z.string().min(1).max(100)).min(1).max(100),book:z.string().trim().min(1).max(500),author:z.string().trim().max(300),chapter:z.string().trim().max(500),locator:z.string().max(300),quote:z.string().trim().min(1).max(100000),interpretation:z.string().trim().min(1).max(100000),at:z.string().max(100),thoughts:z.array(thought).max(1000),libraryBookId:z.string().max(100).optional(),sourceLocation:z.string().max(2000).optional()});
...
const ocrEntry=z.object({libraryBookId:z.string().min(1).max(100),page:z.number().int().positive(),text:z.string().max(200000),at:z.string().max(100)});
...
export async function PUT(req:Request){try{
 ...
 const records=[...data.notes,...data.cards,...data.crossThoughts,...data.readingNotes,...data.thoughtReplies];const allIds=new Set(records.map(n=>n.id));const ids=new Set([...data.notes,...data.cards,...data.crossThoughts].map(n=>n.id));if(allIds.size!==records.length)return response('记录标识重复。',400);
 ...
 const thoughtIds=new Set([...data.notes.flatMap(n=>(n.thoughts||[]).map(t=>t.id)),...data.cards.flatMap(c=>c.thoughts.map(t=>t.id)),...data.crossThoughts.flatMap(c=>c.thoughts.map(t=>t.id)),...data.readingNotes.flatMap(r=>r.thoughts.map(t=>t.id))]);
