...
const readingNote=z.object({id:z.string().min(1).max(100),questionIds:z.array(z.string().min(1).max(100)).max(100),book:z.string().trim().min(1).max(500),author:z.string().trim().max(300),chapter:z.string().trim().max(500),locator:z.string().max(300),quote:z.string().trim().min(1).max(100000),interpretation:z.string().trim().min(1).max(100000),at:z.string().max(100),thoughts:z.array(thought).max(1000),libraryBookId:z.string().max(100).optional(),sourceLocation:z.string().max(2000).optional()});
...
const libraryBook=z.object({id:z.string().min(1).max(100),title:z.string().trim().min(1).max(500),author:z.string().max(300),format:z.enum(['pdf','epub']),storedFile:z.string().min(1).max(500),originalName:z.string().min(1).max(500),addedAt:z.string().max(100),progress:z.object({page:z.number().int().positive().optional(),totalPages:z.number().int().positive().optional(),cfi:z.string().max(4000).optional(),percentage:z.number().min(0).max(1).optional(),rotation:z.union([z.literal(0),z.literal(90),z.literal(180),z.literal(270)]).optional()})});
