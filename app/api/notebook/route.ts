...
const crossThought=z.object({id:z.string().min(1).max(100),anchorIds:z.array(z.string().min(1).max(100)).min(2).max(30),text:z.string().trim().min(1).max(100000),origin:z.string().max(100),source:z.string().max(10000),at:z.string().max(100),thoughts:z.array(thought).max(1000)});
...
export async function PUT(req:Request){try{
 ...
 for(const r of data.readingNotes)if(r.questionIds.some(id=>!data.notes.some(n=>n.id===id&&n.kind==='question')))return response('阅读笔记需要属于有效的问题。',400);
 ...
 for(const c of data.crossThoughts)if(new Set(c.anchorIds).size!==c.anchorIds.length||c.anchorIds.some(id=>!anchorIds.has(id)))return response('交叉思考需要连接至少两个有效位置。',400);
 ...
 for(const c of old.crossThoughts){const next=data.crossThoughts.find(n=>n.id===c.id);if(!next||!same({...c,thoughts:[]},{...next,thoughts:[]})||!same(c.thoughts,next.thoughts.slice(0,c.thoughts.length)))return response('交叉思考和已有记录需要保留，请追加新的思考。',400);}
