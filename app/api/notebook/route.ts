...
const schema=z.object({version:z.number().int().nonnegative(),data:z.object({notes:z.array(note).min(1).max(3000),cards:z.array(card).max(5000),readingNotes:z.array(readingNote).max(10000),links:z.array(z.object({id:z.string(),from:z.string(),to:z.string(),type:z.enum(['相关','支持','反驳','启发'])})).max(10000),dismissedSuggestions:z.array(z.string()).max(15000)})});
